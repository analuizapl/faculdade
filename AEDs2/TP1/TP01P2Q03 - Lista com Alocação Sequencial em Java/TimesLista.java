 	...
 	try{
 		...
 		while(endereco.contains("Capacity")==false) endereco=Arq.readLine();
 				  	...
 				  	capacidadeAux= capacidadeAux.replaceAll("<[^>]*>","");
 						...
 						capacidadeAux.contains("planned"))   {
...
//remover ou inserir na lista de acordo com a entrada
		 for(int j=0;j<numEntrada2;j++){
			 Time t = new Time();
			 linha3=linha2[j].split(" ");

       //inserir
       if(linha3[0].charAt(0)=='I'){
           if(linha3[0].charAt(1)=='I'){					//inserirInicio
               t.ler(linha3[1]);
               lista.inserirInicio(t);
           }else
             if(linha3[0].charAt(1)=='F'){				///inserirFim
                     t.ler(linha3[1]);
                     lista.inserirFim(t);
             }else
               if(linha3[0].charAt(1)=='*'){ 				//inserir
                     t.ler(linha3[2]);
                     int posicao=Integer.parseInt(linha3[1]);
                     lista.inserir(t,posicao);
             }else{

               System.exit(1);
             }
     }else
       if(linha3[0].charAt(0)=='R'){
         if(linha3[0].charAt(1)=='I'){					//removerInicio
             t=lista.removerInicio();
         }else
           if(linha3[0].charAt(1)=='F'){				///removerFim
               t=lista.removerFim();
         }else
           if(linha3[0].charAt(1)=='*'){ 				//remover

                   int posicao=Integer.parseInt(linha3[1]);
                   t=lista.remover(posicao);
           }else{
     System.exit(1);
     }

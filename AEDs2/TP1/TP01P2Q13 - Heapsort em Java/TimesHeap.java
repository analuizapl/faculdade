...
*constroi o heap
*/
    public int constroi(int tamHeap){
      int cont=0;
      for(int i = tamHeap; i > 1 && array[i].getEstadio().compareTo(array[i/2].getEstadio())>0; i /= 2){
         swap(i, i/2);
         cont++;
      }
      return cont;
...
*/
  public int reconstroi(int tamHeap){
      ...
      int i = 1, filho;
      while(i <= (tamHeap/2)){
         if (array[2*i].getEstadio().compareTo(array[2*i+1].getEstadio())>0 || 2*i == tamHeap){
            filho = 2*i;
            cont++;
         } else {
            filho = 2*i + 1;
...
public class TimesHeap{
     ...
     */
           ...
           Arq.openWrite("591514_heapsort.txt","ISO-8859-1");

...
void selecao(int i) {
     ...
     if(i < (n - 1)) {
        int menor = i;
        for (int j = (i + 1); j < n; j++){
           if (strcmp(array[menor].nome,array[j].nome)){
              menor = j;
           }
        }
        swap(menor, i);
...
void quicksortA (int esq, int dir) {
   int i = esq, j = dir;
   Time pivo = array[(dir+esq)/2];
   while (i <= j) {
      while (pstrcmp(array[i].apelido,pivo.apelido)<0) i++;
      while (pstrcmp(array[i].apelido,pivo.apelido)<0) j--;
      if (i <= j) {
         swap(i, j);
         i++;
         j--;
   ...
   if (esq < j)  quicksortA(esq, j);
   if (i < dir)  quicksortA(i, dir);
selecao();
}

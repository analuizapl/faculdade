...
class Lista {
   ...
   public void startMerge(){
      ...
      public void mergeSort( Time [] array ) {
    Time [ ] a = new Time[n];
   ...
   //Metodo recursivo que chama o metodo merge e troca suas posicoes
    public void mergeSort( Time [] array, Time [] tmpArray, int left, int right ) {
        int comparacao=0;
      if( left < right ) {
            comparacao++;
   ...
   */
    ...
    public void merge( Time [] array,Time [] a,int leftPos,int rightPos,int rightEnd ) {
    int leftEnd = rightPos - 1;
    int tmpPos = leftPos;
    int numElements = rightEnd-leftPos + 1;
    while( leftPos <= leftEnd && rightPos <= rightEnd ){
      Time tim = array[leftPos], tim2 = array[rightPos];
      if( tim.getNomeArquivo().compareTo(tim2.getNomeArquivo()) < 0){
        comparacao++;
        a[tmpPos++] = array[leftPos++];
      }else a[tmpPos++] = array[rightPos++];
    }

    while(leftPos <= leftEnd) a[tmpPos++] = array[leftPos++];
    while(rightPos <= rightEnd) a[tmpPos++] = array[rightPos++];

    for( int i = 0; i<numElements; i++,rightEnd-- ) array[rightEnd] =a[ rightEnd ];
   }//Fim metodo

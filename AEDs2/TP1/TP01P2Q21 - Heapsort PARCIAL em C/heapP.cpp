...
void constroi(int tamHeap){
      for(int i = tamHeap; i > 1 && (strcmp(array[i]->estadio,array[i/2]->estadio)>0) ; i /= 2){
         swap(i, i/2);
      }
}
...
void reconstroi(int tamHeap){
   ...
   while(i <= (tamHeap/2)){
      ...
      if (strcmp(array[2*i]->estadio,array[2*i+1]->estadio)>0 || 2*i == tamHeap){
         filho = 2*i;
      } else {
         filho = 2*i + 1;
      }

      if(strcmp(array[i]->estadio,array[filho]->estadio)){
         swap(i, filho);
         i = filho;
      }else{
         i = tamHeap;

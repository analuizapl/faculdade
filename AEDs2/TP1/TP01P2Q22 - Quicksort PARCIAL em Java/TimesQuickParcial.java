...
class Lista {
   ...
   private int quicksort(int esq, int dir) {
       int i = esq, j = dir;
       int cont=0;
       Time pivo = array[(dir+esq)/2];
       while (i <= j) {
           cont++;
           while (array[i].getApelido().compareTo(pivo.getApelido())<0) i++; cont++;
           while (array[j].getApelido().compareTo(pivo.getApelido())>0) j--; cont++;
           if (i <= j) {
               swap(i, j);
               i++;
               j--;
...
public class TimesQuickParcial{
     ...
     */
           ...
           Arq.openWrite("591514_quicksort.txt","ISO-8859-1");
